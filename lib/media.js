...
class BackendDesktop {
  ...
  async comando(acao) {
    // O /toggle-play pausa mas deixa o isPaused do plugin para tras, e o botao
    // congela nas barrinhas. Os endpoints dedicados atualizam o estado certo.
    if (acao === 'playpause') {
      const { dados } = await pedir(`${this.base}/api/v1/song`);
      const pausado = !!dados?.isPaused;
      return this.tentar(pausado ? ['/api/v1/play'] : ['/api/v1/pause', '/api/v1/toggle-play']);
    }
  ...
  // Tocar troca a fila inteira, como o play do proprio YouTube Music.
  //
  ...
  // se perde. Entao primeiro poe a faixa para tocar, e so depois zera o resto -
  // ai o radio que o YTM monta ja nasce dela.
  //
  ...
  async tocar(videoId) {
    ...
    await this.sobrarSoAtual();
    return true;
  }

  // Deixa na fila apenas a faixa que esta tocando. Zerar tudo com DELETE
  // /queue apaga tambem a atual, e sem essa ancora o autoplay do YTM nao monta
  // radio nenhum - a fila fica morta. Com ela sozinha, ele reconstroi.
  // Remove de tras para frente para os indices menores nao deslizarem.
  async sobrarSoAtual() {
    const fila = await this.fila();
    if (!fila || fila.atual < 0) return;
    const manter = fila.itens[fila.atual]?.indice;
    if (manter === undefined) return;
    for (let k = fila.itens.length - 1; k >= 0; k--) {
      const indice = fila.itens[k].indice;
      if (indice !== manter) await this.tentar([`/api/v1/queue/${indice}`], undefined, 'DELETE');
    }
  }
